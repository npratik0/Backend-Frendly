...
// //     it("should create post with empty caption", async () => {
// //       const response = await request(app)
// //         .post("/api/posts")
// //         .set("Authorization", `Bearer ${authToken}`)
// //         .attach("image", Buffer.from("fake-image-data"), "test.jpg");
...
// //       const newCommentId = post!.comments[1]._id.toString();
...
//       const response = await request(app)
//         .delete(`/api/posts/${postId}/comment/${newCommentId.toString()}`)
//         .set("Authorization", `Bearer ${authToken}`);
...
// Mock Cloudinary - ADD THIS AT THE TOP!
jest.mock("../../config/cloudinary.config", () => ({
  __esModule: true,
  default: {
    uploader: {
...
// // Mock multer-storage-cloudinary to prevent actual uploads
// jest.mock("multer-storage-cloudinary", () => ({
//   CloudinaryStorage: jest.fn().mockImplementation(() => ({
//     _handleFile: (req: any, file: any, cb: any) => {
//       cb(null, {
//         path: "https://res.cloudinary.com/demo/test-image.jpg",
...
// // Mock cloudinary config for destroy operations
// jest.mock("../../config/cloudinary.config", () => ({
//   __esModule: true,
//   default: {
//     uploader: {
...
describe("Post Integration Tests", () => {
  ...
  describe("DELETE /api/posts/:postId/comment/:commentId", () => {
    ...
    //   // Create post with comment - MongoDB will auto-generate _id for comment
    //   const post = await PostModel.create({
    //     user: userObjectId,
    //     caption: "Test post",
    //     imageUrl: "https://example.com/image.jpg",
    ...
    //   postId = post._id.toString();
    //   // Get the auto-generated comment _id
    //   commentId = post.comments[0]._id.toString();
    // });
    beforeEach(async () => {
  const userObjectId = new mongoose.Types.ObjectId(userId);
